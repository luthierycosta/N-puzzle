...
func TestGetNeighbors(t *testing.T) {
	...
	assert.Equal(NewState(3), stateA)
	...
	assert.Equal(State{[][]int{
			{1, 2, 3},
			{4, 5, 6},
			{7, 0, 8} }, Pair{1, 2}	}, infDirNeigh[0])
	...
	assert.Equal(State{[][]int{
			{4, 2, 3},
			{0, 5, 6},
			{7, 8, 1} }, Pair{0, 1}	}, supEsqNeigh[0])

	// se o 0 esta na borda, so devem ter 2 jogadas possiveis
	bordaSup := State{
		[][]int {{1, 0, 3}, {4, 5, 6}, {7, 8, 2}}, Pair{1, 0} }
	...
	assert.Equal([]State{
		{[][]int {{1, 5, 3}, {4, 0, 6}, {7, 8, 2}}, Pair{1, 1} },
		{[][]int {{1, 3, 0}, {4, 5, 6}, {7, 8, 2}}, Pair{2, 0} }},
		bordaSupNei1 )
	assert.Equal([]State{
		{[][]int {{0, 1, 3}, {4, 5, 6}, {7, 8, 2}}, Pair{0, 0} },
		{[][]int {{1, 3, 0}, {4, 5, 6}, {7, 8, 2}}, Pair{2, 0} }},
		bordaSupNei2 )
	...
	assert.Equal([]State{
		State{ [][]int{{1, 0, 3}, {4, 2, 6}, {7, 8, 5}}, Pair{1, 0} },
		State{ [][]int{{1, 2, 3}, {4, 8, 6}, {7, 0, 5}}, Pair{1, 2} },
		State{ [][]int{{1, 2, 3}, {4, 6, 0}, {7, 8, 5}}, Pair{2, 1} } },
		meioNeigh)
